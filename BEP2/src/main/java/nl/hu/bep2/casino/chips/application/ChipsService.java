...
public class ChipsService {
    ...
    public Balance depositChips(String username, Long amount) {
        Chips chips = this.findChipsByUsername(username); //find chips object
        chips.deposit(amount);              // do the function from domain
        this.chipsRepository.save(chips);   //save chip;
        return this.showBalanceFor(chips);  //only return balance
    }
